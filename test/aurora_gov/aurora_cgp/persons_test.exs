defmodule AuroraGov.PersonsTest do
  use AuroraGov.DataCase

  alias AuroraGov.Persons

  import AuroraGov.PersonsFixtures
  alias AuroraGov.Persons.{Person, PersonToken}

  describe "get_person_by_email/1" do
    test "does not return the person if the email does not exist" do
      refute Persons.get_person_by_email("unknown@example.com")
    end

    test "returns the person if the email exists" do
      %{id: id} = person = person_fixture()
      assert %Person{id: ^id} = Persons.get_person_by_email(person.email)
    end
  end

  describe "get_person_by_email_and_password/2" do
    test "does not return the person if the email does not exist" do
      refute Persons.get_person_by_email_and_password("unknown@example.com", "hello world!")
    end

    test "does not return the person if the password is not valid" do
      person = person_fixture()
      refute Persons.get_person_by_email_and_password(person.email, "invalid")
    end
    ...
    test "returns the person if the email and password are valid" do
      ...
      assert %Person{id: ^id} =
               Persons.get_person_by_email_and_password(person.email, valid_person_password())
    end
  end
  ...
  describe "get_person!/1" do
    ...
    test "returns the person with the given id" do
      %{id: id} = person = person_fixture()
      assert %Person{id: ^id} = Persons.get_person!(person.id)
    end
  end

  describe "register_person/1" do
    test "requires email and password to be set" do
      {:error, changeset} = Persons.register_person(%{})
    ...
    test "validates email and password when given" do
      {:error, changeset} = Persons.register_person(%{email: "not valid", password: "not valid"})
    ...
    test "validates maximum values for email and password for security" do
      ...
      {:error, changeset} = Persons.register_person(%{email: too_long, password: too_long})
    ...
    test "validates email uniqueness" do
      ...
      {:error, changeset} = Persons.register_person(%{email: email})
      assert "has already been taken" in errors_on(changeset).email

      # Now try with the upper cased email too, to check that email case is ignored.
      {:error, changeset} = Persons.register_person(%{email: String.upcase(email)})
      assert "has already been taken" in errors_on(changeset).email
    end

    test "registers persons with a hashed password" do
      email = unique_person_email()
      {:ok, person} = Persons.register_person(valid_person_attributes(email: email))
  ...
  describe "change_person_registration/2" do
    test "returns a changeset" do
      assert %Ecto.Changeset{} = changeset = Persons.change_person_registration(%Person{})
    ...
    test "allows fields to be set" do
      ...
      changeset =
        Persons.change_person_registration(
          %Person{},
  ...
  describe "change_person_email/2" do
    test "returns a person changeset" do
      assert %Ecto.Changeset{} = changeset = Persons.change_person_email(%Person{})
  ...
  describe "apply_person_email/3" do
    ...
    test "requires email to change", %{person: person} do
      {:error, changeset} = Persons.apply_person_email(person, valid_person_password(), %{})
      assert %{email: ["did not change"]} = errors_on(changeset)
    end

    test "validates email", %{person: person} do
      {:error, changeset} =
        Persons.apply_person_email(person, valid_person_password(), %{email: "not valid"})
    ...
    test "validates maximum value for email for security", %{person: person} do
      ...
      {:error, changeset} =
        Persons.apply_person_email(person, valid_person_password(), %{email: too_long})
    ...
    test "validates email uniqueness", %{person: person} do
      ...
      {:error, changeset} = Persons.apply_person_email(person, password, %{email: email})
    ...
    test "validates current password", %{person: person} do
      {:error, changeset} =
        Persons.apply_person_email(person, "invalid", %{email: unique_person_email()})
    ...
    test "applies the email without persisting it", %{person: person} do
      email = unique_person_email()
      {:ok, person} = Persons.apply_person_email(person, valid_person_password(), %{email: email})
      assert person.email == email
      assert Persons.get_person!(person.id).email != email
    end
  end
    ...
    test "sends token through notification", %{person: person} do
      token =
        extract_person_token(fn url ->
          Persons.deliver_person_update_email_instructions(person, "current@example.com", url)
        end)
  ...
  describe "update_person_email/2" do
    ...
    test "updates the email with a valid token", %{person: person, token: token, email: email} do
      assert Persons.update_person_email(person, token) == :ok
    ...
    test "does not update email with invalid token", %{person: person} do
      assert Persons.update_person_email(person, "oops") == :error
    ...
    test "does not update email if person email changed", %{person: person, token: token} do
      assert Persons.update_person_email(%{person | email: "current@example.com"}, token) == :error
      assert Repo.get!(Person, person.id).email == person.email
      assert Repo.get_by(PersonToken, person_id: person.id)
    end
    ...
    test "does not update email if token expired", %{person: person, token: token} do
      ...
      assert Persons.update_person_email(person, token) == :error
      assert Repo.get!(Person, person.id).email == person.email
      assert Repo.get_by(PersonToken, person_id: person.id)
    end
  end
  ...
  describe "change_person_password/2" do
    test "returns a person changeset" do
      assert %Ecto.Changeset{} = changeset = Persons.change_person_password(%Person{})
      assert changeset.required == [:password]
    end

    test "allows fields to be set" do
      changeset =
        Persons.change_person_password(%Person{}, %{
  ...
  describe "update_person_password/3" do
    ...
    test "validates password", %{person: person} do
      {:error, changeset} =
        Persons.update_person_password(person, valid_person_password(), %{
          password: "not valid",
    ...
    test "validates maximum values for password for security", %{person: person} do
      ...
      {:error, changeset} =
        Persons.update_person_password(person, valid_person_password(), %{password: too_long})
    ...
    test "validates current password", %{person: person} do
      {:error, changeset} =
        Persons.update_person_password(person, "invalid", %{password: valid_person_password()})
    ...
    test "updates the password", %{person: person} do
      ...
      assert is_nil(person.password)
      assert Persons.get_person_by_email_and_password(person.email, "new valid password")
    end

    test "deletes all tokens for the given person", %{person: person} do
      _ = Persons.generate_person_session_token(person)
  ...
  describe "get_person_by_session_token/1" do
    ...
    test "returns person by token", %{person: person, token: token} do
      assert session_person = Persons.get_person_by_session_token(token)
      assert session_person.id == person.id
    end

    test "does not return person for invalid token" do
      refute Persons.get_person_by_session_token("oops")
    end

    test "does not return person for expired token", %{token: token} do
      {1, nil} = Repo.update_all(PersonToken, set: [inserted_at: ~N[2020-01-01 00:00:00]])
      refute Persons.get_person_by_session_token(token)
    end
  end
  ...
  describe "delete_person_session_token/1" do
    test "deletes the token" do
      person = person_fixture()
      token = Persons.generate_person_session_token(person)
      assert Persons.delete_person_session_token(token) == :ok
      refute Persons.get_person_by_session_token(token)
    end
  end
  ...
  describe "confirm_person/1" do
    ...
    test "confirms the email with a valid token", %{person: person, token: token} do
      assert {:ok, confirmed_person} = Persons.confirm_person(token)
    ...
    test "does not confirm with invalid token", %{person: person} do
      assert Persons.confirm_person("oops") == :error
    ...
    test "does not confirm email if token expired", %{person: person, token: token} do
      {1, nil} = Repo.update_all(PersonToken, set: [inserted_at: ~N[2020-01-01 00:00:00]])
      assert Persons.confirm_person(token) == :error
      refute Repo.get!(Person, person.id).confirmed_at
      assert Repo.get_by(PersonToken, person_id: person.id)
    end
  end
  ...
  describe "deliver_person_reset_password_instructions/2" do
    ...
    test "sends token through notification", %{person: person} do
      token =
        extract_person_token(fn url ->
          Persons.deliver_person_reset_password_instructions(person, url)
        end)
  ...
  describe "reset_person_password/2" do
    ...
    test "validates password", %{person: person} do
      {:error, changeset} =
        Persons.reset_person_password(person, %{
          password: "not valid",
          password_confirmation: "another"
        })
    ...
    test "validates maximum values for password for security", %{person: person} do
      too_long = String.duplicate("db", 100)
      {:error, changeset} = Persons.reset_person_password(person, %{password: too_long})
      assert "should be at most 72 character(s)" in errors_on(changeset).password
    end

    test "updates the password", %{person: person} do
      {:ok, updated_person} = Persons.reset_person_password(person, %{password: "new valid password"})
    ...
    test "deletes all tokens for the given person", %{person: person} do
      _ = Persons.generate_person_session_token(person)
      {:ok, _} = Persons.reset_person_password(person, %{password: "new valid password"})
      refute Repo.get_by(PersonToken, person_id: person.id)
    end
  end
