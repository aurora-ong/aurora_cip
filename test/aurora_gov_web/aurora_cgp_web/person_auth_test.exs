defmodule AuroraGovWeb.PersonAuthTest do
  use AuroraGovWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias AuroraGov.Persons
  alias AuroraGovWeb.PersonAuth
  import AuroraGov.PersonsFixtures

  @remember_me_cookie "_aurora_gov_web_person_remember_me"

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, AuroraGovWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_person/3" do
    test "stores the person token in the session", %{conn: conn, person: person} do
      ...
      assert Persons.get_person_by_session_token(token)
    end
  ...
  describe "logout_person/1" do
    test "erases session and cookies", %{conn: conn, person: person} do
      person_token = Persons.generate_person_session_token(person)
      ...
      assert redirected_to(conn) == ~p"/"
      refute Persons.get_person_by_session_token(person_token)
    end

    test "broadcasts to the given live_socket_id", %{conn: conn} do
      live_socket_id = "persons_sessions:abcdef-token"
      AuroraGovWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "fetch_current_person/2" do
    ...
    test "authenticates person from cookies", %{conn: conn, person: person} do
      ...
      assert get_session(conn, :live_socket_id) ==
               "persons_sessions:#{Base.url_encode64(person_token)}"
    end
  ...
  describe "on_mount :redirect_if_person_is_authenticated" do
    ...
    test "doesn't redirect if there is no authenticated person", %{conn: conn} do
      session = conn |> get_session()
